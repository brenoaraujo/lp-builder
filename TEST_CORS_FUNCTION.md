...
## Quick Test to Verify CORS Headers

Open your browser console and run this test to see what CORS headers your function is actually returning:

```javascript
// Test the draft-open function CORS headers
fetch('https://kvtouoigckngalfvzmsp.supabase.co/functions/v1/draft-open/test?token=test', {
  method: 'OPTIONS',
  headers: {
    'Origin': 'https://lp-builder-pi.vercel.app'
  }
})
.then(response => {
...
If the function is properly deployed, you should see:
- `Access-Control-Allow-Origin: https://lp-builder-pi.vercel.app`
- `Access-Control-Allow-Credentials: true`

If you see:
- `Access-Control-Allow-Origin: *` - The function wasn't updated properly

## Manual Deployment Steps

If the test shows the wrong headers, here's the exact deployment process:

### 1. Deploy draft-open Function

1. Go to **Supabase Dashboard** → **Edge Functions**
2. Find `draft-open` function
3. Click **Edit**
...
### 2. Deploy drafts Function

1. Find the `drafts` function
2. Click **Edit**
...
1. **Run the CORS test again** to verify the headers are correct
2. **Clear your browser cache** (Ctrl+Shift+R or Cmd+Shift+R)
3. **Test the magic link** - the CORS error should be gone

## If Still Not Working

If you're still getting CORS errors after deployment:

1. **Check Supabase logs** - Go to Functions → Logs to see if there are any errors
2. **Wait 2-3 minutes** - Sometimes deployments take a moment to propagate
3. **Try in incognito mode** - To rule out caching issues
